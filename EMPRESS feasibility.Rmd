...
events <- events_master_enrol %>%
  filter(event_type_name %in% c(empress_plat_keep, empress_dom_keep)) %>%
  mutate(
    event_time_local = with_tz(timestamp, "Europe/Copenhagen"),
    id = event_id
  ) %>%
  select(id, ssid, event_description, responsible_site, event_type_name, event_time_local)
...
baseline_issues <- anchors %>%
  ...
  transmute(
    ...
    issues = list({
      ...
      icu <- date_of_index_icu_admission
      rdt <- rand_date

      dob_id <- date_of_birth__id
      hosp_id <- date_of_index_hospital_admission__id
      icu_id <- date_of_index_icu_admission__id
      ...
      if (!is.na(icu) && !is.na(dob) && icu < dob) {
        out <- add_issue(
          out, "date_of_index_icu_admission", "hard", icu,
          paste0("ICU admission (", icu, ") before DOB (", dob, ")"),
          icu_id
        )
      }

      if (!is.na(hosp) && !is.na(icu) && icu < hosp) {
        out <- add_issue(
          out, "date_of_index_icu_admission", "hard", icu,
          paste0("ICU admission (", icu, ") before hospital admission (", hosp, ")"),
          icu_id
        )
      }
      ...
      if (!is.na(icu) && !is.na(rdt) && icu > rdt) {
        out <- add_issue(
          out, "date_of_index_icu_admission", "hard", icu,
          paste0("Index ICU admission (", icu, ") is after randomisation date (", rdt, ")"),
          icu_id
        )
      }
      ...
      if (!is.na(icu) && !is.na(rdt) && interval(icu, rdt) > years(1)) {
        out <- add_issue(
          out, "date_of_index_icu_admission", "soft", icu,
          paste0("ICU admission (", icu, ") is > 1 year before randomisation (", rdt, ")"),
          icu_id
        )
      }
